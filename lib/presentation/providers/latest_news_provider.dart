...
final latestNewsProvider =
    StateNotifierProvider<LatestNewsNotifier, List<Article>>(
        (ref) => LatestNewsNotifier(ref));

class LatestNewsNotifier extends NewsNotifier {
  final StateNotifierProviderRef ref;
  LatestNewsNotifier(this.ref) : super();
  ...
  void fetchArticles(BuildContext context) async {
    try {
      state = await Future.delayed(
        ...
        ref.read(mockNewsProvider).getLatestArticles,
      );
    } catch (e) {
  ...
  void markAllRead() async {
    ...
    for (final article in state) {
      newState.add(article.copyWith(read: true));
    }
    state = newState;
  }
